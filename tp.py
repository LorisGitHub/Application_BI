...
# Replace firs file rangadh col missing values with empty string
...
# Replace col ANNEE_DEM missings values with the correct value
...
# newDf["DTNAIS"] = np.where(pd.isnull(newDf["DTNAIS"]) and not np.isnan(newDf["AGEAD"]), (newDf["DTADH"].str[0:4]).astype('int64') - newDf["AGEAD"].astype('int64'), newDf["DTNAIS"])
newDf['DTNAIS'].fillna("1961-07-04", inplace = True)

# Replace col AGEAD missings values with the correct value
...
newDf["agedem"] = np.where(np.isnan(newDf["agedem"]), (newDf["DTNAIS"].str[0:4]).astype('int64') + (newDf["DTDEM"].str[0:4]).astype('int64'), newDf["agedem"])
